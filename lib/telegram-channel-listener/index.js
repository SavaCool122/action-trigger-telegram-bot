...
export async function telegramChannelUpdateListner(client, db, logger) {
  ...
  const handler = async (update) => {
    ...
    if (isValidAria && parsedMessage.type === "START")
      sendStartPost(client, db, logger);
    if (isValidAria && parsedMessage.type === "END")
      sendEndPost(client, db, logger);
    if (isValidAria && !isValidType)
