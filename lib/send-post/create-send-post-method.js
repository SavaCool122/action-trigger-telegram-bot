...
function getFieldByType(type) {
  if (type === "START")
    return { message: "start_message", image: "start_image" };
  else return { message: "end_message", image: "end_image" };
}

function getFullFilePath(fileName) {
  return path.join(import.meta.dirname, "assets", fileName);
}
      ...
      channelsWithData.map(
        async (channel) =>
          await sendMessage(
            ...
            getFullFilePath(channel[dataFieldByType.image])
          )
      )
    );
